...
| EC-08 | GDPR DPA with LLM provider required | §8 — Security Requirements (GDPR Compliance) | DPA required with all five data processors if the system handles real visitor data. **Not a blocker for this project:** the system processes synthetic test data only. DPA execution is deferred to any future commercial deployment. |
